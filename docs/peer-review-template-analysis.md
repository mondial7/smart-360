...
5. ✅ **Likert ratings against a competency rubric** *(shipped)*. Templates can declare a list of `competencies` (key, name, description). When present, every submission must rate the subject 1–5 on each competency with a one-line justification (server rejects empty justifications, duplicates, out-of-range scores, and missing-competency coverage). The submit form renders a 5-button scale + free-text justification per competency. The consolidation persists `competencyRatings` — a deterministic, server-computed aggregate per competency with self score, per-voice averages (manager / peer / report), others average, count, and spread. Rendered in `MyFeedbackView`, `ConsolidationView`, and the PDF as side-by-side bar rows; a wide-spread warning surfaces calibration gaps for the 1:1. The AI prompt receives the raw per-reviewer scores too and is instructed to treat wide spreads as findings rather than to restate the numbers.
6. ✅ **Recency anchor surfaced in the UI** *(shipped)*. A dashed-border aside on the submit form tells reviewers to anchor feedback on the last 3–6 months; the create-round review step warns admins that reviewers will see this prompt. The AI prompt already enforced the same window in earlier steps.
7. ✅ **Private-to-manager channel** *(shipped)*. Optional textarea on peer submissions feeds into a `manager_only_channel` block on the consolidation (synthesis + themes + raw relationship-tagged notes). The block is stripped from API responses, the in-app subject view, and the PDF whenever the caller is not the round creator or a global admin — `canSeeManagerOnlyChannel` centralises that rule.
8. ✅ **Bias / toxicity moderation as a dedicated step** *(shipped)*. A separate Gemini call (`gemini-flash-latest`) runs over every submission before the synthesis prompt sees it. The model is told to *only scrub* — not synthesise, not add — and outputs `{cleaned, flagged, reasons}`. Each pass writes a row to a `moderation_logs` collection (round + submission ID, model, flagged bool, reasons, fields scrubbed, timestamp). The synthesis prompt's instruction set is correspondingly slimmer ("content has already been scrubbed; drop anything you still see silently"). Admins and round creators can read the audit trail via `GET /api/rounds/:id/moderation-logs`.
