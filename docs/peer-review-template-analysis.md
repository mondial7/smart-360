...
4. **Configurable templates per role family** (`template_id` on round; questions/persona/competencies stored as config rather than hard-coded). Lets non-eng teams use the same tool.
