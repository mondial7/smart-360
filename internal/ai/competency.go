...
// ValidateRatings enforces, against the round's template:
//   - if the template defines no competencies, ratings must be empty;
//   - every template competency is rated exactly once;
//   - scores are 1..5 inclusive and justifications are non-empty.
//
// It returns a user-facing error or nil.
...
// AggregateCompetencyRatings produces a deterministic per-competency view of the
// Likert ratings across submissions: self score, per-voice averages, and the
// spread (max minus min across non-self ratings). The template provides the
// canonical order and human-readable name/description.
